...
> Comprehensive guide for configuring and using Fangcun Guard's data leakage prevention system.
...
Fangcun Guard's Data Leakage Prevention (DLP) system provides **multi-layer protection** against sensitive data exposure when using AI models. The system automatically:
...
docker compose restart fangcunguard-admin fangcunguard-proxy
```
...
import logging
logging.getLogger("fangcunguard").setLevel(logging.DEBUG)
```
...
docker logs -f fangcunguard-proxy | grep -i "data_leakage"

# Admin service logs (policy configuration)
docker logs -f fangcunguard-admin | grep -i "policy"

# Database queries (if needed)
docker logs -f fangcunguard-postgres
```
...
A: Fangcun Guard provides **technical controls** for data protection (detection, blocking, anonymization). GDPR compliance depends on:
...
**Community**:
- [GitHub Issues](https://github.com/Fangcun-AI/Fangcun-Guard/issues)
