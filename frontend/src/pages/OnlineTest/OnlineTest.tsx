...
const OnlineTest: React.FC = () => {
  ...
  const getRiskColor = (level: string) => {
    switch (level) {
      ...
      case 'error':
        return 'bg-red-100 text-red-800 border-red-200'
      default:
        return 'bg-zinc-100 text-zinc-800 border-zinc-800'
    }
  }
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <p className="text-zinc-400 mb-6">{t('onlineTest.description')}</p>
            ...
            <CardContent className="space-y-4">
              ...
              <Collapsible>
                <CollapsibleTrigger asChild>
                  <Button variant="outline" size="sm" className="w-full justify-between">
                    ...
                    <span className="text-xs text-zinc-400">{t('onlineTest.clickToExpand')}</span>
                  </Button>
                </CollapsibleTrigger>
                <CollapsibleContent className="mt-3 space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {testCasesByCategory[selectedCategory as keyof typeof testCasesByCategory].map((testCase) => (
                      <Card key={testCase.id} className="cursor-pointer hover:bg-white/5 transition-colors" onClick={() => useTestCase(testCase)}>
                        <CardContent className="p-3 space-y-2">
                          <div className="flex items-center justify-between">
                            <p className="text-sm font-semibold">{testCase.name}</p>
                            <span className="px-2 py-0.5 text-xs rounded bg-indigo-100 text-indigo-800 border border-indigo-200">{testCase.category}</span>
                          </div>
              ...
              {inputType === 'question' && (
                <div>
                  ...
                  {models.length === 0 ? (
                    <div className="p-4 bg-indigo-950/30 border border-indigo-800 rounded-md">
                      <div className="flex items-start gap-2">
                        <div className="flex-shrink-0 mt-0.5">
                          <div className="h-5 w-5 rounded-full bg-indigo-500 flex items-center justify-center text-white text-xs">i</div>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-indigo-200">{t('onlineTest.noProxyModels')}</p>
                          <p className="text-sm text-indigo-300 mt-1">
                            {t('onlineTest.noProxyModelsDesc').split(t('onlineTest.securityGateway'))[0]}
                            <Button variant="link" size="sm" className="h-auto p-0 text-indigo-400" onClick={() => navigate('/security-gateway')}>
                          ...
                          <Card className="bg-zinc-800/50">
                              ...
                              <CardHeader className="py-3">
                                <div className="flex items-center justify-between w-full">
                                  <div className="flex items-center gap-2">
                                    ...
                                    {model.selected && <span className="px-2 py-0.5 text-xs rounded bg-indigo-100 text-indigo-800 border border-indigo-200">{t('onlineTest.selected')}</span>}
                                  </div>
                                  <Switch
                            ...
                            <CollapsibleContent>
                              <CardContent className="pt-0 pb-3 space-y-3">
                                <div>
                                  <p className="text-xs text-zinc-400 mb-1">{t('onlineTest.apiBaseUrl')}</p>
                                  <p className="text-xs text-zinc-200">{model.api_base_url}</p>
                                </div>

                                {model.selected && (
                                  <div>
                                    <p className="text-xs text-zinc-400 mb-1">{t('onlineTest.modelNameLabel')}</p>
                                    <Input
                                      size={1}
                  ...
                  {models.filter((m) => m.selected).length > 0 && (
                    <p className="text-xs text-indigo-400 mt-2">{t('onlineTest.selectedModels', { count: models.filter((m) => m.selected).length })}</p>
                  )}
                </div>
              )}

              <div className="flex gap-2">
                <Button onClick={runTest} disabled={loading} size="lg" className="bg-indigo-600 hover:bg-indigo-700">
                  {loading ? (
                    <>
            ...
            <CardContent className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <Card>
                          ...
                          <CardContent className="space-y-2">
                            <div>
                              <span className="text-xs text-zinc-400">{t('onlineTest.riskLevel')} </span>
                            ...
                            {testResult.guardrail.security?.categories?.length > 0 && (
                              <div>
                                <span className="text-xs text-zinc-400">{t('onlineTest.riskCategory')} </span>
                      ...
                      <Card className="border-zinc-800 bg-zinc-800/50">
                        <CardContent className="pt-4">
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                              <p className="text-sm font-medium text-zinc-200 mb-1">{t('onlineTest.overallRiskLevel')}</p>
                            ...
                            <div>
                              <p className="text-sm font-medium text-zinc-200 mb-1">{t('onlineTest.suggestedAction')}</p>
                            ...
                            <div>
                              {testResult.guardrail.suggest_answer && (
                                <>
                                  <p className="text-sm font-medium text-zinc-200 mb-1">{t('onlineTest.suggestedAnswer')}</p>
                                  <code className="text-xs bg-zinc-900 px-3 py-1.5 rounded border border-zinc-800 block whitespace-pre-wrap break-all">{testResult.guardrail.suggest_answer}</code>
                                </>
                              )}
                            </div>
                          </div>
                ...
                {inputType === 'question' && Object.keys(testResult.original_responses).length > 0 && (
                  <div>
                    ...
                    <div className="p-3 bg-indigo-950/30 border border-indigo-800 rounded-md mb-4 text-sm text-indigo-300">{t('onlineTest.proxyModelOriginalResponseDesc')}</div>
                          ...
                          <Card key={modelId}>
                            ...
                            <CardContent>
                              {response.error ? (
                                ...
                                <div>
                                  <p className="text-xs font-semibold text-zinc-200 mb-2">{t('onlineTest.originalResponse')}</p>
                                  <div className="bg-zinc-800/50 p-3 rounded-md border border-zinc-800">
                                    <p className="text-sm whitespace-pre-wrap">{response.content}</p>
                                  </div>
                                </div>
                              ) : (
                                <p className="text-sm text-zinc-400">{t('onlineTest.emptyResponse')}</p>
                              )}
                            </CardContent>
                          </Card>
                        )
                          ...
                          <Card key={modelId}>
                            ...
                            <CardContent>
                              {response.error ? (
                                ...
                                <div>
                                  <p className="text-xs font-semibold text-zinc-200 mb-2">{t('onlineTest.modelResponse')}</p>
                                  <div className="bg-zinc-800/50 p-3 rounded-md border border-zinc-800">
                                    <p className="text-sm whitespace-pre-wrap">{response.content}</p>
                                  </div>
                                </div>
        ...
        <TabsContent value="batch">
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-zinc-400">{t('onlineTest.batchTest.description')}</p>
                <p className="text-xs text-zinc-500">{t('onlineTest.batchTest.formatRequirement')}</p>
                ...
                <div className="space-y-4">
                  ...
                  {batchStatus === 'idle' && !batchFile ? (
                    <label
                      ...
                      className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-zinc-800 rounded-lg cursor-pointer bg-zinc-800/50 hover:bg-white/5 transition-colors"
                    >
                      <Upload className="h-8 w-8 text-zinc-500 mb-2" />
                      <span className="text-sm text-zinc-400">{t('onlineTest.batchTest.uploadArea')}</span>
                      <span className="text-xs text-zinc-500 mt-1">{t('onlineTest.batchTest.uploadHint')}</span>
                    </label>
                  ) : (
                    <div className="p-4 bg-zinc-800/50 rounded-lg border border-zinc-800">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium text-zinc-200">{batchFile?.name}</p>
                  ...
                  <div className="flex gap-2">
                    {batchStatus === 'uploaded' && (
                      <Button onClick={runBatchDetection} className="bg-indigo-600 hover:bg-indigo-700">
                        <PlayCircle className="h-4 w-4 mr-2" />
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-zinc-800">
                          <th className="text-left py-2 px-3 font-medium text-zinc-200">#</th>
                          <th className="text-left py-2 px-3 font-medium text-zinc-200">{t('onlineTest.batchTest.resultColumns.prompt')}</th>
                          <th className="text-left py-2 px-3 font-medium text-zinc-200">{t('onlineTest.batchTest.resultColumns.response')}</th>
                          <th className="text-left py-2 px-3 font-medium text-zinc-200">{t('onlineTest.batchTest.resultColumns.overallRiskLevel')}</th>
                          <th className="text-left py-2 px-3 font-medium text-zinc-200">{t('onlineTest.batchTest.resultColumns.securityRiskLevel')}</th>
                          <th className="text-left py-2 px-3 font-medium text-zinc-200">{t('onlineTest.batchTest.resultColumns.complianceRiskLevel')}</th>
                          <th className="text-left py-2 px-3 font-medium text-zinc-200">{t('onlineTest.batchTest.resultColumns.dataRiskLevel')}</th>
                          <th className="text-left py-2 px-3 font-medium text-zinc-200">{t('onlineTest.batchTest.resultColumns.action')}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {batchResults.slice(0, 50).map((result, index) => (
                          <tr key={index} className="border-b border-zinc-800 hover:bg-white/5">
