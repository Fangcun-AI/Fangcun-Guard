  ...
  const columns: ColumnDef<UserType>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-2">
            <code className="text-xs bg-zinc-800/50 px-2 py-1 rounded">{id.substring(0, 8)}...</code>
            <Button
              variant="ghost"
              size="sm"
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <span className="text-sm text-zinc-400">{new Date(date).toLocaleString()}</span>
      },
    },
    {
      id: 'actions',
            ...
            <CardHeader className="flex-shrink-0">
              <div className="flex justify-between items-start">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-zinc-400">{t('admin.manageTenants')}</p>
                  {adminStats && (
                  ...
                  {analyticsLoading ? (
                    ...
                    <div className="space-y-2 max-h-56 overflow-y-auto">
                      {tenantAnalytics.latest_created_tenants.map((tenant) => (
                        <div
                          key={tenant.id}
                          className="flex items-center justify-between p-1.5 border rounded-md hover:bg-white/5 text-sm"
                        >
                    ...
                    <div className="text-center text-zinc-400 py-6 text-sm">{t('admin.noData')}</div>
                  )}
                </CardContent>
              </Card>
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSave)} className="space-y-4">
              ...
              {editingUser && (
                <>
                  <div className="flex items-center justify-between border rounded-lg p-3 bg-zinc-800/50">
                    <FormLabel>{t('admin.superAdmin')}</FormLabel>
                    <Switch checked={!!editingUser?.is_super_admin} disabled />
                  </div>
                  <div className="border rounded-lg p-3 bg-zinc-800/50">
                    <FormLabel className="mb-2 block">{t('admin.apiKey')}</FormLabel>
                    <div className="flex items-center gap-2">
                      <code 
                        className="flex-1 min-w-0 text-xs bg-zinc-900 px-3 py-2 rounded border border-zinc-800 overflow-hidden text-ellipsis whitespace-nowrap"
