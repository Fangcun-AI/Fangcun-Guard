...
const LLMProviders: React.FC = () => {
  ...
  const columns: ColumnDef<LLMProvider>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-zinc-400">{row.original.provider || '-'}</span>
      ),
    },
    {
          ...
          {row.original.is_private_model ? (
            <>
              ...
              {row.original.is_default_private_model && (
                <Badge variant="outline" className="text-indigo-400 border-indigo-700">
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSave)} className="space-y-4">
              ...
              <div className="space-y-3 p-4 border rounded-lg bg-green-950/20">
                <div>
                  <p className="font-medium mb-1">{t('gateway.privateModelConfig')}</p>
                  <p className="text-sm text-zinc-400">{t('gateway.privateModelConfigDesc')}</p>
                </div>
                  ...
                  <>
                    ...
                    {switchStates.is_default_private_model && modelNames.length > 0 && (
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-zinc-400">{t('gateway.defaultPrivateModelNameDesc')}</p>
                        <Select
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-zinc-400">{t('gateway.privateModelNamesDesc')}</p>
          ...
          {viewingProvider && (
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-2 py-2 border-b">
                <span className="font-medium text-zinc-200">{t('gateway.providerName')}</span>
                <span className="col-span-2">{viewingProvider.config_name}</span>
              </div>

              <div className="grid grid-cols-3 gap-2 py-2 border-b">
                <span className="font-medium text-zinc-200">{t('gateway.providerType')}</span>
              ...
              <div className="grid grid-cols-3 gap-2 py-2 border-b">
                <span className="font-medium text-zinc-200">{t('gateway.privateModel')}</span>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {viewingProvider.private_model_names.map((name) => (
                            <Badge
                              key={name}
                              variant="outline"
                              className={`text-xs ${name === viewingProvider.default_private_model_name ? 'bg-indigo-950/30 border-indigo-700 text-indigo-400' : ''}`}
                            >
                              {name}
