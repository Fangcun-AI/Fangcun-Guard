...
const ProxyModelManagement: React.FC = () => {
    ...
    if (model) {
      ...
      if (modelDetail) {
        ...
        setSwitchStates({
          ...
          reasoning_format: modelDetail.reasoning_format || 'auto',
  ...
  const handleSave = async (values: any) => {
    try {
      ...
      const formData: any = {
        ...
        reasoning_format: switchStates.reasoning_format,
  ...
  return (
    <div className="space-y-4">
      <div className="p-4 bg-indigo-950/50 border border-indigo-800 rounded-lg">
        <p className="font-semibold text-indigo-200">{t('proxy.securityGatewayInfo')}</p>
        <p className="text-sm text-indigo-300 mt-1">{t('proxy.securityGatewayInfoDesc')}</p>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('proxy.accessFangcunGuardGateway')}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="p-4 bg-indigo-950/50 border border-indigo-800 rounded-lg">
            <p className="text-sm text-indigo-200">{t('proxy.gatewayIntegrationDesc')}</p>
          </div>

          <div>
            <p className="font-semibold mb-2">{t('proxy.pythonOpenaiExample')}</p>
            <pre className="bg-zinc-800/50 p-4 rounded-lg overflow-auto text-sm border border-zinc-800">
              <code>
{`client = OpenAI(
    base_url="https://api.fangcunguard.com/v1/gateway",  # Change to FangcunGuard Official gateway url or use your local deployment url http://your-server:5002/v1
    ...
    # Other parameters are the same as the original usage method
          ...
          <div>
            ...
            <ul className="list-disc list-inside space-y-1 text-sm text-zinc-400">
              <li><code className="px-2 py-1 bg-zinc-800/50 rounded text-xs">{t('proxy.dockerDeployment')}</code></li>
              <li><code className="px-2 py-1 bg-zinc-800/50 rounded text-xs">{t('proxy.customDeployment')}</code></li>
            </ul>
          </div>
        </CardContent>
      </Card>
            ...
            <form onSubmit={form.handleSubmit(handleSave)} className="space-y-4">
              ...
              <div className="space-y-3 p-4 border rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-zinc-400">{t('proxy.securityConfigDesc')}</p>
                </div>
                ...
                {switchStates.enable_reasoning_detection && (
                  <div className="flex items-center justify-between">
                    <Label htmlFor="reasoning_format">{t('proxy.reasoningFormat')}</Label>
                    <select
                      id="reasoning_format"
                      value={switchStates.reasoning_format}
                      onChange={(e) => setSwitchStates(prev => ({ ...prev, reasoning_format: e.target.value }))}
                      className="w-48 rounded-md border border-zinc-800 bg-zinc-900 px-3 py-1.5 text-sm"
                    >
                      <option value="auto">{t('proxy.reasoningFormatAuto')}</option>
                      <option value="field">{t('proxy.reasoningFormatField')}</option>
                      <option value="tag">{t('proxy.reasoningFormatTag')}</option>
                      <option value="none">{t('proxy.reasoningFormatNone')}</option>
                    </select>
                  </div>
                )}
              ...
              <div className="space-y-3 p-4 border rounded-lg bg-green-950/30">
                <div>
                  <p className="font-medium mb-1">{t('proxy.dataSafetyLabel')}</p>
                  <p className="text-sm text-zinc-400">{t('proxy.dataSafetyDesc')}</p>
                </div>

                <div className="flex items-center justify-between">
                  <div>
                    <Label htmlFor="is_private_model">{t('proxy.isPrivateModel')}</Label>
                    <p className="text-sm text-zinc-400">{t('proxy.isPrivateModelDesc')}</p>
                  </div>
                  <Switch
                    id="is_private_model"
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-zinc-400">{t('proxy.isDefaultPrivateModelDesc')}</p>
                  </div>
                  <Switch
              ...
              <div>
                ...
                <Input
                  ...
                  className="mt-2"
                />
                <p className="text-sm text-zinc-400 mt-1">{t('proxy.streamDetectionIntervalTooltip')}</p>
              </div>
          ...
          {viewingModel && (
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-2 py-2 border-b">
                <span className="font-medium text-zinc-200">{t('proxy.proxyModelName')}</span>
                <span className="col-span-2">{viewingModel.config_name}</span>
              </div>

              <div className="grid grid-cols-3 gap-2 py-2 border-b">
                <span className="font-medium text-zinc-200">{t('proxy.upstreamApiModelName')}</span>
                <span className="col-span-2">{viewingModel.model_name}</span>
              </div>

              <div className="grid grid-cols-3 gap-2 py-2 border-b">
                <span className="font-medium text-zinc-200">{t('proxy.status')}</span>
              ...
              <div className="grid grid-cols-3 gap-2 py-2 border-b">
                <span className="font-medium text-zinc-200">{t('proxy.securityConfig')}</span>
                <div className="col-span-2 space-y-2">
                  ...
                  {viewingModel.enable_reasoning_detection && viewingModel.reasoning_format && (
                    <Badge variant="secondary" className="bg-indigo-900/50 text-indigo-300 border-indigo-800">
                      {t(`proxy.reasoningFormat`)}: {viewingModel.reasoning_format}
                    </Badge>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 py-2">
                <span className="font-medium text-zinc-200">{t('proxy.createTime')}</span>
                <span className="col-span-2">
