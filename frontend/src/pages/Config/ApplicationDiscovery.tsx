...
const ApplicationDiscovery: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-white">{t('applicationManagement.discovery.title')}</h1>
        <p className="text-zinc-400 mt-1">{t('applicationManagement.discovery.description')}</p>
      </div>

      {/* Overview Alert */}
      <Alert className="bg-indigo-950/50 border-indigo-800">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          {displayedApiKey && (
            <Button
              ...
              className="text-red-500 hover:text-red-400 border-red-800 hover:bg-red-950/50"
            >
        ...
        <CardContent>
          {newApiKey && (
            <Alert className="mb-4 bg-green-950/30 border-green-800">
              <AlertDescription className="text-green-300">
                {t('applicationManagement.discovery.newApiKeyWarning')}
              </AlertDescription>
            </Alert>
          )}
          <div className="flex items-center gap-2 bg-zinc-800/50 rounded-md p-3 font-mono text-sm">
              ...
              {recentDiscoveredApps.map((app) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {app.external_id && (
                      <span className="text-xs text-zinc-500">({app.external_id})</span>
                    )}
                  </div>
      ...
      <div className="flex gap-4">
        ...
        <Button
          ...
          onClick={() =>
            window.open(
              'https://github.com/fangcunguard/fangcunguard/blob/main/docs/THIRD_PARTY_GATEWAY_INTEGRATION.md',
              '_blank'
            )
          }
        >
