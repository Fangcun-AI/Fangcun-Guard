...
const OfficialScannersManagement: React.FC = () => {
  ...
  const packageColumns: ColumnDef<Package>[] = [
    ...
    },
    {
      ...
      header: 'Bundle',
      cell: ({ row }) => (
        <Badge variant="secondary" className="bg-indigo-900/50 text-indigo-300 border-indigo-800 text-xs">
          {row.original.bundle || '-'}
        </Badge>
      ),
    },
          ...
          {paymentLoading ? (
            <div className="py-10 text-center space-y-6">
              ...
              <div className="space-y-3">
                ...
                <p className="text-sm text-zinc-400">{t('payment.processing.pleaseWait', '请稍候，请勿关闭页面或刷新')}</p>
              </div>
            </div>
          ) : (
            <>
              ...
              {purchasePackage && (
                <div className="space-y-4">
                  <div className="grid grid-cols-3 gap-2 py-2 border-b">
                    <span className="font-medium text-zinc-200">{t('scannerPackages.packageName')}</span>
                    <span className="col-span-2">{purchasePackage.package_name}</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 py-2 border-b">
                    <span className="font-medium text-zinc-200">{t('scannerPackages.author')}</span>
                    <span className="col-span-2">{purchasePackage.author}</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 py-2 border-b">
                    <span className="font-medium text-zinc-200">{t('scannerPackages.version')}</span>
                    <span className="col-span-2">{purchasePackage.version}</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 py-2 border-b">
                    <span className="font-medium text-zinc-200">{t('scannerPackages.scannerCount')}</span>
                    <span className="col-span-2">{purchasePackage.scanner_count}</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 py-2 border-b">
                    <span className="font-medium text-zinc-200">Bundle</span>
                    <span className="col-span-2">{purchasePackage.bundle || '-'}</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 py-2 border-b">
                    <span className="font-medium text-zinc-200">{t('scannerPackages.priceDisplay')}</span>
                  ...
                  {purchasePackage.description && (
                    <div className="grid grid-cols-3 gap-2 py-2">
                      <span className="font-medium text-zinc-200">{t('scannerPackages.description')}</span>
          ...
          {selectedPackage && (
            <div className="mt-6 space-y-6">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2 py-2">
                  <span className="font-medium text-zinc-200">{t('scannerPackages.enabled')}</span>
