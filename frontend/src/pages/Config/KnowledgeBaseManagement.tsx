  ...
  const columns: ColumnDef<KnowledgeBase>[] = [
    {
      ...
      cell: ({ row }) => <Badge variant="secondary" className="bg-indigo-900/50 text-indigo-300 border-indigo-800">{getCategoryLabel(row.original)}</Badge>,
    },
    {
      accessorKey: 'name',
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Badge variant={row.original.is_global ? 'secondary' : 'default'} className={row.original.is_global ? 'bg-indigo-900/50 text-indigo-300 border-indigo-800' : ''}>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {row.original.is_global ? (
            <>
              ...
              {user?.is_super_admin && (
                <Button variant="ghost" size="sm" className="text-red-500 hover:text-red-400 hover:bg-red-950/50" onClick={() => handleDelete(row.original)}>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <div>
            ...
            <p className="text-sm text-zinc-400 mt-1">{t('knowledge.knowledgeBaseDescription')}</p>
          </div>
        ...
        <CardContent className="space-y-4">
          <div className="p-4 bg-indigo-950/50 border border-indigo-800 rounded-lg">
            <p className="font-medium text-indigo-200 mb-2">{t('knowledge.fileFormatDescription')}</p>
            <p className="text-sm text-indigo-300 mb-2">{t('knowledge.fileFormatDetails')}</p>
            <pre className="bg-zinc-800/50 p-2 rounded text-xs overflow-auto">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
              ...
              {selectedScannerType === 'marketplace_scanner' && availableScanners.marketplace_scanners.length === 0 && (
                <div className="p-4 bg-indigo-950/50 border border-indigo-800 rounded-lg">
                  <p className="font-medium text-indigo-200 mb-1">{t('knowledge.noPurchasedScannersTitle') || 'No Purchased Premium Scanners'}</p>
              ...
              {!editingItem && (
                <div>
                  ...
                  <div className="mt-2">
                    ...
                    />
                    {selectedFile && <p className="text-sm text-zinc-400 mt-1">{selectedFile.name}</p>}
                  </div>
                </div>
              )}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-2">
                ...
                />
                {replaceFile && <p className="text-sm text-zinc-400 mt-1">{replaceFile.name}</p>}
              </div>
            </div>
          </div>
          <DialogFooter>
