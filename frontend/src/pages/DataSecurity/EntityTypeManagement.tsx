  ...
  const columns: ColumnDef<EntityType>[] = [
    {
      ...
      cell: ({ row }) => (
        <code className="text-xs bg-zinc-800/50 px-1.5 py-0.5 rounded whitespace-nowrap">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
              {editingEntity && editingEntity.source_type === 'system_copy' && (
                <div className="flex items-start gap-2 p-3 bg-indigo-950/50 border border-indigo-800 rounded-lg">
                  <Info className="h-4 w-4 text-indigo-400 mt-0.5" />
                  <p className="text-sm text-indigo-200">{t('entityType.systemCopyEditHint')}</p>
                </div>
              )}
                        ...
                        <FormControl>
                          <div className="relative">
                            <Input
                              ...
                              className={!editingEntity ? 'bg-zinc-800/50' : ''}
                            />
              ...
              <div className="space-y-4">
                ...
                {needsSubscription() && (
                  <Alert className="bg-amber-950/50 border-amber-800">
                    ...
                    <AlertTitle className="text-amber-200">{t('entityType.premiumFeatureTitle')}</AlertTitle>
                ...
                {form.watch('recognition_method') === 'regex' ? (
                  <div className="space-y-4 p-4 border rounded-lg bg-indigo-950/20">
                    <FormField
                      control={form.control}
                    ...
                    <div className="flex items-start gap-2 p-3 bg-amber-950/50 border border-amber-800 rounded-lg">
                      ...
                      <div className="text-sm space-y-1">
                        <p className="font-medium text-amber-200">{t('entityType.testInputWarning')}</p>
                        <p className="text-amber-300">{t('entityType.testInputHint')}</p>
                        <ul className="text-amber-300 space-y-0.5 pl-4">
                    ...
                    {recognitionTestResult && (
                      <div className={`p-3 border rounded-lg ${recognitionTestResult.error ? 'bg-red-950/50 border-red-800' : recognitionTestResult.matched ? 'bg-green-950/50 border-green-800' : 'bg-yellow-950/50 border-yellow-800'}`}>
                        {recognitionTestResult.error ? (
                          <p className="text-sm text-red-400">{recognitionTestResult.error}</p>
                        ) : recognitionTestResult.matched ? (
                          <div>
                            <p className="text-sm font-medium text-green-400 mb-1">{t('entityType.recognitionTestMatched', { count: recognitionTestResult.matches.length })}</p>
                            <div className="flex flex-wrap gap-1">
                              {recognitionTestResult.matches.map((match, idx) => (
                                <code key={idx} className="text-xs bg-green-900/50 text-green-400 px-2 py-0.5 rounded">{match}</code>
                              ))}
                            </div>
                          </div>
                        ) : (
                  ...
                  <div className="space-y-4 p-4 border rounded-lg bg-green-950/20">
                    <FormField
                      control={form.control}
                    ...
                    {entityDefinitionTestResult && (
                      <div className={`p-3 border rounded-lg ${entityDefinitionTestResult.error ? 'bg-red-950/50 border-red-800' : entityDefinitionTestResult.matched ? 'bg-green-950/50 border-green-800' : 'bg-yellow-950/50 border-yellow-800'}`}>
                        {entityDefinitionTestResult.error ? (
                          ...
                          <div>
                            <p className="text-sm font-medium text-green-400 mb-1">{t('entityType.entityDefinitionTestMatched', { count: entityDefinitionTestResult.matches.length })}</p>
                            <div className="flex flex-wrap gap-1">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4 p-4 border rounded-lg bg-purple-950/20">
                    <FormField
                      control={form.control}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <p className="text-xs text-zinc-400 mt-1">{t('entityType.replaceTextHint')}</p>
                            <FormMessage />
                          </FormItem>
                        )}
                      />
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <FormField control={form.control} name="regex_replacement" render={({ field }) => (
                            <FormItem>
                              ...
                              <p className="text-xs text-zinc-400 mt-1">{t('entityType.regexReplacementHint')}</p>
                              <FormMessage />
                            </FormItem>
                          )} />
                        </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <Card className="bg-green-950/50 border-green-800">
                          <CardContent className="p-3">
                            <p className="text-xs font-semibold text-green-200 mb-2">{t('entityType.genaiAnonymizationExamplesTitle')}</p>
                            <ul className="text-xs text-green-400 space-y-1 list-none">
                      ...
                      <div className="space-y-4">
                        ...
                        {generatedGenaiCode && (
                          <div className="border rounded-lg overflow-hidden">
                            <button
                              ...
                              className="w-full flex items-center justify-between p-3 bg-zinc-800/50 hover:bg-zinc-800/50 transition-colors"
                            >
                            ...
                            {showGenaiCode && (
                              <div className="p-3 bg-zinc-900 overflow-x-auto">
                        ...
                        {generatedGenaiCode && (
                          <>
                            ...
                            {genaiCodeTestResult && (
                              <div className={`p-3 border rounded-lg ${genaiCodeTestResult.error ? 'bg-red-950/50 border-red-800' : 'bg-green-950/50 border-green-800'}`}>
                                {genaiCodeTestResult.error ? (
                                  <p className="text-sm text-red-400">{genaiCodeTestResult.error}</p>
                                ) : (
                                  <div>
                                    <p className="text-sm font-medium text-green-400 mb-1">{t('entityType.genaiCodeAnonymizedResult')}</p>
                                    <code className="text-sm bg-zinc-900/50 text-green-400 px-2 py-1 rounded block">{genaiCodeTestResult.anonymized_text}</code>
                                  </div>
                                )}
                              </div>
                            )}
                        ...
                        <Card className="bg-purple-950/50 border-purple-800">
                          <CardContent className="p-3">
                            <p className="text-xs font-semibold text-purple-200 mb-2">{t('entityType.genaiCodeExamplesTitle')}</p>
                    ...
                    <div className="pt-4 border-t">
                      ...
                      {['regex_replace', 'genai_natural'].includes(form.watch('anonymization_method') || '') && (
                        <div className="flex items-start gap-2 p-3 bg-amber-950/50 border border-amber-800 rounded-lg mb-4">
                          <Info className="h-4 w-4 text-amber-600 mt-0.5 flex-shrink-0" />
                          <div className="text-sm space-y-1">
                            <p className="font-medium text-amber-200">{t('entityType.testInputWarning')}</p>
                            <p className="text-amber-300">{t('entityType.testInputHint')}</p>
                      ...
                      />
                      {testResult && (
                        <div className="mt-2 p-3 bg-zinc-900 border rounded-lg">
                          <p className="text-sm font-medium text-zinc-200">{t('entityType.testResult')}:</p>
                          <code className="text-sm text-green-400 bg-green-950/50 px-2 py-1 rounded mt-1 block">{testResult}</code>
                        </div>
                      )}
                    </div>
                    )}
                  ...
                  <FormField control={form.control} name="is_global" render={({ field }) => (
                    <FormItem className="flex items-center justify-between border rounded-lg p-3">
                      <div className="flex items-center gap-2">
                        ...
                        <Info className="h-4 w-4 text-zinc-500" title={t('entityType.systemConfigTooltip')} />
                      </div>
                      <FormControl><Switch checked={field.value} onCheckedChange={field.onChange} /></FormControl>
                    </FormItem>
                  )} />
