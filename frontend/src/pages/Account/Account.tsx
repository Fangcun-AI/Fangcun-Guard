...
const Account: React.FC = () => {
  ...
  const handleCopyDifyEndpoint = async () => {
    const endpoint = 'https://api.fangcunguard.com/v1/dify/moderation'
    try {
          ...
          >
            ...
            <TabsContent value="general" className="space-y-6 mt-6">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-zinc-400">{t('account.email')}</label>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <div className="flex-1 p-3 border border-zinc-800 rounded-md bg-zinc-800/50 font-mono text-sm break-all">
                    {user?.id || '-'}
                  </div>
                  <Button
                    variant="outline"
                ...
                <p className="text-sm text-zinc-400">{t('account.uuidNote')}</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="p-4 border border-zinc-800 rounded-md bg-zinc-800/50">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <div className="flex-1 p-3 border border-zinc-800 rounded-md bg-zinc-800/50 font-mono text-sm break-all">
                    https://api.fangcunguard.com/v1/dify/moderation
                  </div>
                ...
                <p className="text-sm text-zinc-400">{t('account.difyModerationEndpointNote')}</p>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-zinc-400 mt-1">
                    {t('docs.directModelAccessDesc') ||
                      'Use this API key to directly access models (Qwen3Guard-Gen-8B, bge-m3, etc.) without guardrails detection. For privacy, we only track usage count, not content.'}
                  </p>
                ...
                {(() => {
                  ...
                  return (
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <div
                          className={`flex-1 p-3 border border-zinc-800 rounded-md font-mono text-sm break-all ${
                            hasAccess ? 'bg-zinc-800/50' : 'bg-zinc-800/50'
                          }`}
                        >
                          {hasAccess ? (
                            user?.model_api_key
                ...
                <div className="space-y-2">
                  ...
                  <pre
                    className={`bg-zinc-800/50 p-4 rounded-md overflow-auto text-xs leading-relaxed ${
                      user?.model_api_key &&
                      (!isSaasMode() ||
                        user?.is_super_admin ||
                        subscription?.subscription_type === 'subscribed')
...
response = client.chat.completions.create(
    model="Qwen3Guard-Gen-8B",  # or bge-m3
...
# Privacy Notice: Content is NOT logged, only usage count`}
                ...
                <div className="p-4 bg-indigo-900/20 border border-indigo-800 rounded-lg">
                  ...
                  <ul className="space-y-1 text-sm text-indigo-300 list-disc pl-5">
                    <li>
                ...
                <div className="p-4 bg-yellow-900/20 border border-yellow-800 rounded-lg space-y-3">
                  <div>
                    ...
                    {subscription ? (
                      <div className="space-y-3">
                        ...
                        <div>
                          ...
                          <span className="text-sm text-zinc-400"> {t('account.calls')}</span>
                        </div>
                        <Progress
                          value={Math.min(subscription.usage_percentage, 100)}
                          className={`h-2 ${subscription.usage_percentage >= 90 ? '[&>div]:bg-red-500' : '[&>div]:bg-indigo-500'}`}
                        />
                        <p className="text-xs text-zinc-400">
                      ...
                      <div className="p-3 bg-yellow-900/20 border border-yellow-800 rounded">
                        <p className="text-sm text-yellow-200">
                          {t('account.subscriptionNotFound', {
                            email: systemInfo?.support_email || 'support@fangcunguard.com',
                          })}
                        </p>
                      </div>
                    ) : (
              ...
              {systemInfo?.support_email && (
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-zinc-400">{t('account.fangcunguardServices')}</p>
