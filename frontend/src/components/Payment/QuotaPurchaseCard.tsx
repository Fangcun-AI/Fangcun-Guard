  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="p-4 rounded-lg border bg-zinc-800/50">
          <p className="text-sm font-medium text-zinc-400 mb-2">{t('billing.purchasedQuota')}</p>
          {hasQuota ? (
            <div className="space-y-1">
              <p className="text-2xl font-bold text-green-600">
                {currentQuota.toLocaleString()} <span className="text-sm font-normal text-zinc-500">{t('billing.calls')}</span>
              </p>
            ...
            <p className="text-zinc-500">{isExpired ? t('billing.quotaExpired') : t('billing.noPurchasedQuota')}</p>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-zinc-400">{t('billing.quotaUnits')}</span>
          ...
          <div className="flex gap-4 text-sm">
            <div>
              <span className="text-zinc-400">{t('billing.quotaTotalCalls')}: </span>
              <span className="font-semibold">{totalCalls.toLocaleString()}</span>
            </div>
            <div>
              <span className="text-zinc-400">{t('billing.quotaTotalPrice')}: </span>
              <span className="font-semibold text-indigo-400">¥{totalPrice}</span>
            </div>
          </div>
        </div>
