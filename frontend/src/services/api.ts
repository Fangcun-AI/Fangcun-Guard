...
export const pluginsApi = {
  ...
  }> => api.get('/api/v1/plugins').then(res => res.data),

  // Get single plugin details
  get: (name: string): Promise<any> =>
    api.get(`/api/v1/plugins/${name}`).then(res => res.data),
};

// Skill Scanner API
export const skillScannerApi = {
  scan: (data: { tools: any[]; policy_mode?: string }): Promise<any> =>
    api.post('/api/v1/skill-scanner/scan', data).then(res => res.data),
  scanDirectory: (data: { directory: string; max_tools?: number }): Promise<any> =>
    api.post('/api/v1/skill-scanner/scan-directory', data).then(res => res.data),
};

// MCP Scanner API
export const mcpScannerApi = {
  scan: (data: { servers: any[]; policy_mode?: string }): Promise<any> =>
    api.post('/api/v1/mcp-scanner/scan', data).then(res => res.data),
  scanDirectory: (data: { directory: string }): Promise<any> =>
    api.post('/api/v1/mcp-scanner/scan-directory', data).then(res => res.data),
};
