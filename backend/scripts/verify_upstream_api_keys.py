...
def main():
    ...
    try:
        ...
        for config in configs:
            ...
            try:
                ...
                if decrypted_key.startswith('sk-xxai-'):
                    print(f"  ⚠️  Warning: This API Key looks like an FangcunGuard platform API Key (sk-xxai-)")
                    print(f"      Upstream API Key should be the API Key for the upstream service (e.g. OpenAI)")
                    print(f"      Not the API Key for accessing the FangcunGuard platform")
                    issues_found = True
        ...
        if issues_found:
            ...
            print()
            print("Explanation:")
            print("  • FangcunGuard API Key (sk-xxai-xxx): Used for client access to the FangcunGuard platform")
            print("  • Upstream API Key (e.g. sk-xxx): Stored in the configuration, used for FangcunGuard to call the upstream service")
            print()
