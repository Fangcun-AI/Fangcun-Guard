...
_PLUGINS_BUILTIN_DIR = Path(__file__).resolve().parents[2] / "plugins_builtin"
...
def _to_plugin_info(name: str, plugin) -> PluginInfo:
...
async def list_plugins():
    ...
    plugins = [_to_plugin_info(name, plugin) for name, plugin in all_plugins.items()]
...
async def get_plugin(plugin_name: str):
    ...
    return _to_plugin_info(plugin_name, plugin)
