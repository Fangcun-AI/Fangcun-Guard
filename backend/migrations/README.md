...
This directory contains database migration scripts for FangcunGuard.
...
docker exec -it fangcunguard-admin python3 migrations/run_migrations.py

# Dry run (show pending migrations without executing)
docker exec -it fangcunguard-admin python3 migrations/run_migrations.py --dry-run
```
...
cat backend/migrations/versions/002_fix_ban_policy_risk_level.sql | \
  docker exec -i fangcunguard-postgres psql -U fangcunguard -d fangcunguard
```
...
docker exec fangcunguard-postgres psql -U fangcunguard -d fangcunguard \
...
docker logs fangcunguard-admin | grep -i migration
```
