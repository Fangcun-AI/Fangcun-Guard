...
class DataSecurityService:
    ...
    async def detect_sensitive_data(
        ...
        # 3. Filter to regex entity types only
        regex_entity_types = [et for et in entity_types if et.get('recognition_method', 'regex') == 'regex']

        logger.info(f"Entity types: {len(regex_entity_types)} regex")

        detected_entities = []
        highest_risk_level = 'no_risk'
        detected_categories = set()

        # 4. Regex detection (full text)
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        future = executor.submit(asyncio.run, self.general_llm.chat(messages))
                        result = future.result(timeout=30)
                else:
                    result = loop.run_until_complete(self.general_llm.chat(messages))
            except RuntimeError:
                # No event loop, create a new one
                result = asyncio.run(self.general_llm.chat(messages))
...
{{"regex_pattern": "your_pattern_here", "replacement_template": "your_replacement_here", "explanation": "brief_explanation"}}"""
        ...
        try:
            ...
            result = await self.general_llm.chat(messages)
